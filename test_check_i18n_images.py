...
""",
    ...
    def test_write_html_report_contains_summary_and_clickable_images(self):
        ...
        with tempfile.TemporaryDirectory() as td:
            ...
            self.assertIn("report-container", content)
            self.assertIn("width:min(100% - 32px, 1440px)", content)
            self.assertIn("@media (max-width: 640px)", content)
            self.assertIn(".table-wrap { overflow-x:auto;", content)
            ...
            self.assertNotIn("max-height:calc(100vh - 160px)", content)
            self.assertNotIn(".table-wrap { overflow:auto;", content)
    ...
    def test_write_html_report_excludes_new_no_text_detail_rows_but_keeps_summary(self):
        ...
        with tempfile.TemporaryDirectory() as td:
            ...
            self.assertIn("tr.mainland-new-with-text[title],tr.mainland-new-with-chars[title] { cursor:help; }", content)
